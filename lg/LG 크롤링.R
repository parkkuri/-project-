...
all.url<-c() # url이 잘 뽑혔는지 확인하기 위해 url 넣을 곳 설정
all.text  <- c() # 자기소개서 텍스트 넣을 곳 설정

urlall <- "http://www.jobkorea.co.kr/Starter/PassAssay/View/" # url 앞부분 변수에 넣기
for(EEE in 147148:146808){ # 페이지번호를 돌려서 페이지를 추출, EEE가 페이지번호
  ...
  if(str_detect(b,'LG' )==F){ # <title>에 LG가 없으면 next, 있으면 추출
    next
  }else{
    tx <- html_nodes(jasoju, "div.tx") #   <div class="tx"> 코드 안에 있는 글귀 tx 변수에 넣기 

    text <- html_text(tx[!str_detect(tx, "href")])  # <div class="tx"><a href (...)로 시작하는 코드에 있는 글귀 빼기기
...
write.csv(all.text,'LG.csv') # 디렉토리에 csv로 저장
